...
const ForgotEmailSending = () => {
  ...
  const forgotFunction = async (details) => {
    ...
    try {
      const response = await fetch("https://url-shortner-backend-itr5.onrender.com/forgot", {
        method: "POST",
...
const ValidateUrl = () => {
  ...
  useEffect(() => {
    const validatingUrl = async () => {
      try {
        const response = await fetch(`https://url-shortner-backend-itr5.onrender.com/forgot/${params.id}`, {
          method: "GET"
        })
        const data = await response.json()
